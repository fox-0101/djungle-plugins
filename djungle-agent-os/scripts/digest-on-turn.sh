...
# DB session_id: marcatore agentos-session (ADR-014b, l'ultimo vince), con
# fallback legacy per i transcript nati prima del plugin 4.7. Convenzione e
# implementazione uniche: skills/_shared/session-threading.md + lib-session.sh.
. "$(dirname "$0")/lib-session.sh" 2>/dev/null || exit 0
DB_SID="$(agentos_session_id "$TRANSCRIPT_PATH")"
[ -z "$DB_SID" ] && exit 0
